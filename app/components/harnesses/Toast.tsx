/**
 * Toast Component Harness
 *
 * A notification toast component built on @radix-ui/react-toast.
 * Provides accessible toast notifications with auto-dismiss and swipe-to-dismiss.
 *
 * STRUCTURE:
 * - Toast viewport (container for all toasts)
 * - Individual toast with optional icon, title, description, action, and close button
 * - Variants for different notification types (default, success, error, warning)
 *
 * STYLE .MD SHOULD CUSTOMIZE:
 * - Viewport: position (top-right, bottom-right, etc.), spacing
 * - Toast container: background, border, radius, shadow, padding
 * - Variant colors: success (green), error (red), warning (yellow), info (blue)
 * - Title: font-size, font-weight, color
 * - Description: font-size, color
 * - Icon: size, color per variant
 * - Close button: size, color, position, hover state
 * - Animation: enter/exit transitions, slide direction
 */
...
function Toast({
  ...
  return (
    <ToastPrimitive.Root
      ...
      className={`flex items-start gap-3 p-4 border rounded-md shadow-md ${getVariantStyles(variant)}`}
    >
      {/* Variant icon */}
      ...
      {action && (
        <ToastPrimitive.Action altText={action.label} asChild>
          <button
            ...
            className="flex-shrink-0 text-sm font-medium text-gray-700 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 rounded"
          >
            {action.label}
          </button>
      ...
      <ToastPrimitive.Close
        className="flex-shrink-0 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 rounded"
        aria-label="Close"
      >
        <svg
          width="15"
