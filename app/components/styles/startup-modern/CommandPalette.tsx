/**
 * CommandPalette - Startup Modern Style
 *
 * Clean command palette with subtle shadow and rounded corners.
 * Yellow-600 for selected state, smooth keyboard navigation.
 */
...
function CommandPaletteContent({
  ...
  return (
    <Command
      className={`w-full bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden ${className}`}
      loop
    >
      {/* Search input */}
      <div className="flex items-center gap-3 px-4 border-b border-gray-200">
        ...
        <Command.Input
          ...
          className={`w-full py-4 text-sm bg-transparent border-0 outline-none placeholder:text-gray-400 ${inputClassName}`}
        />
      </div>
          ...
          >
            {group.items.map((item) => (
              <Command.Item
                ...
                className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 rounded-lg cursor-pointer transition-colors duration-100 aria-selected:bg-yellow-50 aria-selected:text-yellow-900 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed"
              >
                {/* Optional icon */}
                {item.icon && (
                  <span className="text-gray-500 aria-selected:text-yellow-700">{item.icon}</span>
                )}
                ...
                {/* Keyboard shortcut */}
                {item.shortcut && (
                  <kbd className="px-2 py-1 text-xs font-mono text-gray-500 bg-gray-100 border border-gray-200 rounded-md">
                    {item.shortcut}
                  </kbd>
                )}
              </Command.Item>
