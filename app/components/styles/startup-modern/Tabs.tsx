/**
 * Tabs - Startup Modern Style
 *
 * Clean tab navigation with yellow-600 active indicator.
 * Smooth transitions, comfortable spacing.
 */
    ...
    >
      ...
      >
        {tabs.map((tab) => (
          <TabsPrimitive.Trigger
            ...
            className={`px-4 py-2.5 text-sm font-medium text-gray-600 transition-all duration-150 hover:text-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-yellow-600 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed data-[state=active]:text-yellow-700 data-[state=active]:border-b-2 data-[state=active]:border-yellow-600 data-[state=active]:-mb-px ${triggerClassName}`}
          >
            {tab.label}
          </TabsPrimitive.Trigger>
        ))}
      ...
      {tabs.map((tab) => (
        <TabsPrimitive.Content
          key={tab.id}
          value={tab.id}
          className={`pt-6 focus:outline-none ${panelClassName}`}
        >
          {tab.content}
        </TabsPrimitive.Content>
      ))}
